...
(function(element) {
    ...
    require(['d3', 'vue'], function(d3, Vue) {
        ...
        woo = function callEverything() {
            //console.log(argList); //JESSE
            textfile = argList[2]; //JESSE
            csvfile = d3.csvParse(argList[1]); //JESSE
            //textfile = document.getElementById("py-tree").value; //"data/"+datadate + "-tree.txt"; //Steve
            //console.log("tree file: ", textfile, textfileJ);
            //csvfile = d3.csvParse(document.getElementById("py-csv").value); //"data/"+datadate + "-performance.csv"; //Steve
            //console.log("Steve's csvfile: ", csvfile);
            //console.log("Jesse's csvfile: ", csvfileJ);
            if (treeExists) {
        ...
        function analyze(error, treeformat, perfdata) {
            ...
            perfdata.map(function(d){
               ...
               if (avgTime > 0) {
                   domainTimes.push(+d.time); //katy check times * 1.e-9);
                   domainTimesIn.push(+d.time); //+d.time); //kttime
               }
...
//                 domainTimesIn.push(+d.time); //kttime
        ...
        threshold =  0;
            ...
            g.append("text")
                ...
                .attr("y", -6)
                .text( "Total time");//"Average time per instance"); //this isn't right KATY
        }
        function flatten(d) {
          ...
          var node = svg.selectAll('g.node')
              .data(nodes, function(d) {
                  ...
                  if (prim_inst.indexOf(nodename) >= 0) {
                    ...
                    //console.log("time: ", node_perfdata.avg_time);
             ...
             if (d.children) {
                 for (child of d.children) {
                     ...
                     if (child._perfdata.time > d.childrenTime) {
                         ...
                         //console.log("611 ",d.childrenTime, d._perfdata);
                     }
                 }
             }
             if (d._perfdata) {
                 ...
                 //console.log("621 ", d._perfdata.inclusiveTime, d._perfdata.avgTime);
             }
         });
...
//                 }
        ...
        // Add labels for the nodes
          nodeEnter.append('text')
              ...
              .text(function(d) { 
                  if (!d.children) {
                      if (d._perfdata) return d._perfdata.display_name;              
                  } else {
                      return "";
                  }
              });
        ...
        function makeCodeArray() { //JESSE
            ...
            */
            //codeText = document.getElementById("py-src").value; //Steve
            codeText = argList[3]; //JESSE
        ...
        // START OF CODE
        console.log("HERE  -- KATY TRYING TO MAKE VIS OUTPUT AREA BIGGER");
        ...
        tool_tip = d3.tip()
              .attr("class", "d3-tip")
              .offset([-200,-85]) //-255, -85 //-200, -100 good when in iframe looking thing
//               .style("z-index", "-1")
              .html(function(d){ 
                 if (d.id === 1) {
        ...
        /*tool_tip_south = d3.tip()
                .attr('class', 'd3-tip-s')
                .offset([-75,-90])//[-90,-250]) //maybe have a function here? -8
                .html(function(d){ 
                  if (d.id === 1) {
                      return "<p>Function: " + d.data.name + "</p>";
                  }
